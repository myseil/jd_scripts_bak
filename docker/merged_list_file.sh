...
35 1,22 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX));  . /scripts/docker/auto_help.sh export > /scripts/logs/auto_help_export.log && node  /scripts/jd_nzmh.js |ts >> /scripts/logs/jd_nzmh.log 2>&1
...
# 领京豆额外奖励(每日可获得3京豆)
23 */1 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX));  . /scripts/docker/auto_help.sh export > /scripts/logs/auto_help_export.log && node  /scripts/jd_bean_home.js |ts >> /scripts/logs/jd_bean_home.log 2>&1
# 微信小程序京东赚赚
...
# 必须要的默认定时任务请勿删除
2 0 * * * docker_entrypoint.sh |ts >> /scripts/logs/default_task.log 2>&1
